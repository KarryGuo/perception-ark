...
import { createAccount, getAccountByUsername, getAccountById, getAccountByPhone, updateAccountProfile, updateAccountUsername, isUsernameTaken, suggestAvailableUsernames, deleteAccount, getSecurityQuestion, verifySecurityAnswer, resetPassword, updateSecurity, addFamilyBinding, getFamilyBindings, removeFamilyBinding, getActiveFamilyBindingsAsContacts, confirmFamilyBinding, rejectFamilyBinding, getPendingConfirmViBindings, addLoginLog, updateAccountLastLogin } from '../services/memory-store.js';
...
// 用户名修改冷却期: 半年 = 180 天
const USERNAME_COOLDOWN_MS = 180 * 24 * 60 * 60 * 1000;
// 用户名最小长度: 3 个字符
const USERNAME_MIN_LENGTH = 3;
const USERNAME_MAX_LENGTH = 20;

/**
 * 实时检查用户名是否可用(用于前端输入校验)
 * GET /api/auth/username/check?username=xxx
 * 返回: { success, available, reason, suggestions: [] }
 *   - available: true=可用 / false=不可用
 *   - reason: 不可用原因(长度不足/已被占用/与当前相同)
 *   - suggestions: 当被占用时,返回5个可用建议
 */
router.get('/username/check', authRequired, async (req, res) => {
  try {
    const { username } = req.query;
    if (!username || !String(username).trim()) {
      return res.json({ success: true, available: false, reason: '请输入用户名', suggestions: [] });
    }
    ...
    if (trimmed.length < USERNAME_MIN_LENGTH) {
      return res.json({
        success: true,
        available: false,
        reason: `用户名不能少于${USERNAME_MIN_LENGTH}个字符`,
        suggestions: [],
      });
    }
    if (trimmed.length > USERNAME_MAX_LENGTH) {
      return res.json({
        success: true,
        available: false,
        reason: `用户名不能超过${USERNAME_MAX_LENGTH}个字符`,
        suggestions: [],
      });
    }
    // 与当前用户名相同
    const current = await getAccountById(req.user.id);
    if (current && current.username === trimmed) {
      return res.json({
        success: true,
        available: false,
        reason: '与当前用户名相同,无需修改',
        suggestions: [],
      });
    }
    // 唯一性校验
    const taken = await isUsernameTaken(trimmed, req.user.id);
    if (taken) {
      // 已被占用: 生成可用建议
      const suggestions = await suggestAvailableUsernames(trimmed, req.user.id, 5);
      return res.json({
        success: true,
        available: false,
    ...
    // 可用
    return res.json({ success: true, available: true, reason: '用户名可用', suggestions: [] });
  } catch (err) {
    log('AUTH', `检查用户名可用性失败: ${err.message}`, 'error');
    res.status(500).json({ success: false, error: err.message });
  }
});
  ...
  try {
    ...
    if (trimmed.length < USERNAME_MIN_LENGTH || trimmed.length > USERNAME_MAX_LENGTH) {
      return res.status(400).json({
        success: false,
        error: `用户名长度需${USERNAME_MIN_LENGTH}-${USERNAME_MAX_LENGTH}个字符`,
      });
    }
    // 半年修改一次限制
    const current = await getAccountById(req.user.id);
    ...
    if (current.last_username_change_at) {
      const lastChange = new Date(current.last_username_change_at).getTime();
      ...
      if (elapsed < USERNAME_COOLDOWN_MS) {
        ...
        return res.status(429).json({
          success: false,
          error: `用户名半年内只能修改一次,距下次可修改还需 ${remainingDays} 天(可于 ${nextDate.toLocaleDateString('zh-CN')} 后再次修改)`,
          cooldown: {
            lastChange: current.last_username_change_at,
            nextChangeAt: new Date(lastChange + USERNAME_COOLDOWN_MS).toISOString(),
            remainingDays,
          },
        });
      }
    ...
    if (current.username === trimmed) {
      return res.status(400).json({ success: false, error: '新用户名与当前相同,无需修改' });
    }
    // 唯一性校验(排除当前用户自身)
    const taken = await isUsernameTaken(trimmed, req.user.id);
    if (taken) {
    ...
    // 写入数据库并记录修改时间
    const ok = await updateAccountUsername(req.user.id, trimmed);
    if (!ok) {
