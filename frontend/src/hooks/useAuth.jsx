...
export function AuthProvider({ children }) {
  ...
  // 更新用户信息并同时刷新token(用于用户名修改场景,后端会签发新token)
  const updateUserWithToken = useCallback((newUser, newToken) => {
    if (newToken) {
      secureStorage.setSync('ark_token', newToken);
    }
    setUser(newUser);
  }, []);
  ...
  return (
    <AuthContext.Provider value={{ user, loading, login, loginBySms, register, logout, refreshUser, updateUser, updateUserWithToken }}>
      {children}
