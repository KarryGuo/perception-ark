...
直接输出JSON,绝对不要用markdown代码块包裹,不要有任何解释文字`);
  ...
  // 唤醒状态小芯片(合并进右上"在线"药丸,与顶部药丸导航栏水平对齐,不再遮挡)
  const wakeChip = useMemo(() => {
    if (!firstUse && wakeDialogue) return { cls: 'listening', text: '我在听', aria: '小舟正在听您说话，请说出指令' };
    if (!firstUse && !wakeDialogue && wakeListeningUi && voiceWakeActive) return { cls: '', text: '小舟小舟', aria: '语音唤醒监听中，请说小舟小舟' };
    if (!firstUse && !wakeDialogue && !wakeListeningUi && !wakePermDeniedUi && voiceWakeActive) return { cls: 'off', text: '恢复中', aria: '语音唤醒暂时离线，正在自动恢复' };
    if (wakePermDeniedUi) return { cls: 'denied', text: '麦克风未授权', aria: '麦克风权限被拒绝，语音唤醒不可用' };
    return null;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [firstUse, wakeDialogue, wakeListeningUi, voiceWakeActive, wakePermDeniedUi]);
  ...
  return (
    <div className="am-app ark-dark">
      ...
      <div className="am-top-tools">
        ...
        {wakeChip && (
          <div className={`am-wake-indicator ${wakeChip.cls}`} role="status" aria-label={wakeChip.aria}>
            <span className="am-wake-dot" /><span className="am-wake-text">{wakeChip.text}</span>
          </div>
        )}
        {activeTab === 'recognize' && (
          <>
