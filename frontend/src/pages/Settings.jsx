...
export default function Settings() {
  const { user, logout, updateUser, updateUserWithToken } = useAuth();
  ...
  const [usernameCheckResult, setUsernameCheckResult] = useState(null); // { available, reason, suggestions }
  const [usernameSaving, setUsernameSaving] = useState(false);
  const [usernameCooldown, setUsernameCooldown] = useState(null); // { remainingDays, nextChangeAt } | null
  ...
  const handleUsernameInputChange = useCallback((e) => {
    ...
    if (trimmed.length < 3) {
      setUsernameCheckResult({ available: false, reason: '用户名不能少于3个字符', suggestions: [] });
      return;
    }
    if (trimmed.length > 20) {
      setUsernameCheckResult({ available: false, reason: '用户名不能超过20个字符', suggestions: [] });
      return;
    }
    // 与当前用户名相同
    if (trimmed === user?.username) {
      setUsernameCheckResult({ available: false, reason: '与当前用户名相同,无需修改', suggestions: [] });
      return;
    }
    // 防抖请求后端检查唯一性
    ...
    usernameCheckTimer.current = setTimeout(async () => {
      try {
        ...
        // 网络错误时不阻塞输入,仅静默
        console.warn('[Settings] 用户名检查失败:', err.message);
      } finally {
        setUsernameChecking(false);
      }
    }, 500);
  ...
  // 点击建议用户名: 填入输入框并触发校验
  const handlePickSuggestion = useCallback((suggestion) => {
    setUsernameInput(suggestion);
    // 模拟输入事件触发校验
    handleUsernameInputChange({ target: { value: suggestion } });
  }, [handleUsernameInputChange]);

  // 保存用户名: 调用后端修改接口(已通过实时校验,理论上不会失败,但兜底处理冷却/重名)
  const handleUsernameSave = useCallback(async () => {
    const trimmed = usernameInput.trim();
    if (trimmed.length < 3 || trimmed.length > 20) {
      setAccountMsg({ type: 'err', text: '用户名长度需3-20个字符' });
      return;
    }
    if (trimmed === user?.username) {
      setAccountMsg({ type: 'err', text: '新用户名与当前相同' });
      return;
    }
    ...
    try {
      ...
      if (res?.success && res.user) {
        // 用户名修改后后端签发新token,需同时更新本地token和用户信息
        updateUserWithToken(res.user, res.token);
        setAccountMsg({ type: 'ok', text: '用户名修改成功' });
        setShowUsernameForm(false);
        setUsernameCheckResult(null);
        speak('用户名修改成功');
      } else {
        setAccountMsg({ type: 'err', text: res?.error || '用户名修改失败' });
      }
    } catch (err) {
      // 结构化错误: 可能携带 cooldown / suggestions
      const body = err.body || {};
      if (err.status === 429 && body.cooldown) {
        setUsernameCooldown(body.cooldown);
        setAccountMsg({ type: 'err', text: err.message });
      } else if (err.status === 409 && Array.isArray(body.suggestions) && body.suggestions.length > 0) {
        // 后端返回的可用建议,同步到校验结果展示
        setUsernameCheckResult({
          available: false,
          reason: '该用户名已被占用,可使用以下建议',
          suggestions: body.suggestions,
        });
        setAccountMsg({ type: 'err', text: '该用户名已被占用,请从下方建议中选择' });
      } else {
        setAccountMsg({ type: 'err', text: err.message || '用户名修改失败' });
      }
    } finally {
      setUsernameSaving(false);
    }
  }, [usernameInput, user?.username, updateUserWithToken, speak]);
            ...
            <div className="sp-section-body">
              ...
              <div className="sp-item">
                ...
                {!showUsernameForm ? (
                  ...
                  <div className="sp-username-form">
                    <div className="sp-username-hint">半年内仅可修改一次,请谨慎选择</div>
                    <input
                      type="text"
                      className="sp-input"
                      value={usernameInput}
                      onChange={handleUsernameInputChange}
                      placeholder="新用户名(3-20个字符)"
                      maxLength={20}
                    ...
                    {/* 实时校验状态 */}
                    {usernameChecking && (
                      <div className="sp-username-checking">正在检查用户名可用性...</div>
                    )}
                    {usernameCheckResult && !usernameChecking && (
                      <div className={`sp-username-result ${usernameCheckResult.available ? 'ok' : 'err'}`}>
                        <span className="sp-username-status">
                          {usernameCheckResult.available ? '✓ ' : '✗ '}{usernameCheckResult.reason}
                        </span>
                        {usernameCheckResult.suggestions && usernameCheckResult.suggestions.length > 0 && (
                          <div className="sp-username-suggestions">
                            <span className="sp-username-suggestions-title">可用用户名建议:</span>
                            <div className="sp-username-suggestions-list">
                              {usernameCheckResult.suggestions.map((s) => (
                                <button
                                  key={s}
                                  type="button"
                                  className="sp-username-suggestion-chip"
                                  onClick={() => handlePickSuggestion(s)}
                                  aria-label={`使用建议用户名 ${s}`}
                                >
                                  {s}
                                </button>
                              ))}
                    ...
                    {/* 冷却期提示 */}
                    {usernameCooldown && (
                      <div className="sp-username-cooldown">
                        距下次可修改还需 {usernameCooldown.remainingDays} 天
                        {usernameCooldown.nextChangeAt && ` ( ${new Date(usernameCooldown.nextChangeAt).toLocaleDateString('zh-CN')} 后 )`}
                      </div>
                    )}
                    <div className="sp-pwd-actions">
                      <button
                        ...
                        disabled={usernameSaving || usernameChecking || usernameCheckResult?.available !== true || !usernameInput.trim() || usernameInput.trim() === user?.username}
                      >
                        {usernameSaving ? '保存中' : '确认修改'}
                      </button>
                      <button
