...
async function request(url, options = {}) {
  ...
  if (!res.ok) {
    ...
    // 将完整错误体附加到 Error 上,方便调用方获取 suggestions/cooldown 等字段
...
export const api = {
  ...
  // 实时检查用户名是否可用(返回 available/reason/suggestions)
  checkUsername: (username) =>
    request(`/auth/username/check?${new URLSearchParams({ username }).toString()}`),
  // 修改登录用户名(半年一次,需唯一)
